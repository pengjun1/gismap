  ...
  <div class="addMap">
    ...
    <div class="bottom" v-if="startFlag==3">
      <div class="bg">
        ...
        <div class="block">
          <span class="demonstration" :style="'color:'+mapColor">选择轮廓颜色</span>
          <el-color-picker v-model="mapColor" :predefine="predefineColors" @change="changeColor"></el-color-picker>
        </div>
        <div class="btn">
    ...
    <div class="bottom" v-else>
      <div :class="{'startDraw':true,'back':startFlag==1}" @click="btnDraw">{{startFlag==1 ? '⮐ 撤销' : '点击开始绘制'}}</div>
      <!-- <div class="text">
        <div>
          显示已绘制的地块
        </div>
...
export default {
  ...
  data(){
    return {
      ...
      polygon:'',//多边形
      isShow:true,//显示地块
      // startFlag: this.$route.query.startFlag | 0,// 绘制状态: 0:未开始,1:开始绘制,2:绘制结束
      mapColor: this.$route.query.fieldsInfo ? JSON.parse(this.$route.query.fieldsInfo).mapColor : '#EB7D3B',//当前颜色
      predefineColors: [
        '#EC7E3B',
        '#FFFD72',
        '#99F667',
  ...
  mounted(){
    this.init()
    if(this.$route.query.startFlag == 3) {
      JSON.parse(this.$route.query.fieldsInfo).lnglats.filter(item => {
  ...
  methods:{
    ...
    clickToDraw(e){
      ...
      this.textMarker = new AMap.Text({
        text:this.path.length==1 ? '继续打点' : '点击第一个点结束绘制',
    ...
    endDraw(){
      ...
      this.polygon = new AMap.Polygon({
        path: this.path,  
        fillColor: this.mapColor, // 多边形填充颜色
        borderWeight: 1, // 线条宽度，默认为 1
        strokeColor: this.mapColor, // 线条颜色
        fillOpacity:.5
      });
    ...
    saveAndQuit(){
        ...
        // 获取地块面积
        var mapArea = (AMap.GeometryUtil.ringArea(this.path)/666.666666).toFixed(2)
        ...
        AMap.plugin('AMap.Geocoder', function () {
            ...
            geocoder.getAddress(center, function (status, result) {
                if (status === 'complete' && result.info === 'OK') {
                    ...
                    // 地块信息对象
                    var addMapData = {
                        lnglats:JSON.stringify(_this.path),
                        mapColor:_this.mapColor,
                    ...
                    _this.$emit('getNewMap',addMapData)
                }
            })
        })
    }
